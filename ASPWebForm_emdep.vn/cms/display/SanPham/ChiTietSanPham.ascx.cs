...
namespace ASPWebForm_emdep.vn.cms.display.SanPham
{
    public partial class ChiTietSanPham : System.Web.UI.UserControl
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = ASPWebForm_emdep.vn.App_Code.Database.Size.Thongtin_Size_by_id(SizeID);
            if (dt.Rows.Count > 0)
        ...
        {
            DataTable dt = new DataTable();
            dt = ASPWebForm_emdep.vn.App_Code.Database.Mau.Thongtin_Mau_by_id(MauID);
            if (dt.Rows.Count > 0)
        ...
        {
            DataTable dt = new DataTable();
            dt = ASPWebForm_emdep.vn.App_Code.Database.ChatLieu.Thongtin_ChatLieu_by_id(ChatLieuID);
            if (dt.Rows.Count > 0)
